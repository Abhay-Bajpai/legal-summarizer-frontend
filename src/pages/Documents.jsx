...
export default function DocumentsPage(){
  ...
  useEffect(()=>{
    axios.get('http://localhost:8000/api/documents/').then(r=>setDocs(r.data)).catch(()=>{})
  },[])
  return (
    <div>
        ...
        {docs.map(d=> (
          <div key={d.id} className="p-4 bg-white dark:bg-gray-800 rounded shadow">
            <div className="flex justify-between items-center">
              ...
              <a className="text-sm text-blue-600" href={`#`} onClick={(e)=>{e.preventDefault(); alert(d.summary)}}>View summary</a>
            </div>
          </div>
        ))}
      </div>
